...
class DataHubTestCase(unittest.TestCase):
    ...
    def test_005_cleanup(self):
        ...
        try:
            ...
            self._cursor.execute(
                " delete pgp from pg.PostingGroupProcessing pgp join pg.PostingGroup pg on pgp.PostingGroupId = pg.PostingGroupId where PostingGroupCode = 'PUBR01-SUBR01-PUBN01-ACCT'")
    ...
    def test_010_insert_publisher(self):
        ...
        :return:
        """
        print('test_010_insert_publisher')
        try:
    ...
    def test_020_insert_subscriber_SUBR01(self):
        ...
        :return:
        """
        print('test_020_insert_subscriber_SUBR01')
        try:
    ...
    def test_050_posting_group_created(self):
        ...
        :return:
        """
        print('test_050_posting_group_created')
        try:
    ...
    def test_070_file_name_issue_lookup(self):
        print('test_070_file_name_issue_lookup')
        # Test 1 get issue data based on File name.
        print('\r\n****    Test 1 get issue data based on File name.')
        sql = 'select max(IssueId) IssueId' \
              ' from ctl.Issue d ' \
              ' where d.IssueName=\'PUBN01-ACCT-00001-19000101\''
        print(sql,'\r\n')
        ...
        params={"FileName":"PUBN01-ACCT-00001-19000101"}

        dh.get_publication_list(params)  

        self.assertEqual(test_issue_id, dh.get_issue_id(), 'File Name Issue Lookup Failed.')

        # Test 2 Update Issue to Status IL.
        print('\r\n****    Test 2 set issue status to IL.')

        params['StatusCode']='IL'
        dh.update_issue(params)

        sql = 'select count(1) RecCount from [pg].[vw_PostingGroupProcessingStatus]' \
              ' where PostingGroupCode = \'PUBR01-SUBR01-PUBN01-ACCT\''
        
        print(sql)
        
        self._cursor.execute(sql)
        record_count = self._cursor.fetchall()
        pgp_record_count = int(record_count[0]['RecCount'])
        self._test_db_connection.commit()

        self.assertEqual(pgp_record_count, 1, 'Posting Group Processing (notify subscriber of dist) creation failed.')

    def test_080_posting_group_processing_created(self):
        """
        Attempt to create a new Posting Group  record.
        Updates Issue status, notify subscriber of distribution.
        :return:
