...
def log_ui_operation(
    ...
    try:
        # Get or create step logger in session state
        if "step_logger" not in st.session_state:
            try:
                from snowflake.snowpark.context import get_active_session
                session = get_active_session()
                
                st.session_state["step_logger"] = StepLoggerSnowflake(
                    session=session,
                    etl_execution_id=str(uuid.uuid4()),
                    process_name="Streamlit_UI_Operations",
                    process_description="Streamlit UI operations logging",
                    process_type="UI_OPERATIONS"
                )
            except Exception as e:
                logger.error(f"Failed to create StepLoggerSnowflake: {e}")
                return False
        
        step_logger = st.session_state["step_logger"]
        
        # Prepare step name and custom attributes
        ...
        custom_attrs = {
            ...
            "UI_Session": str(uuid.uuid4())
        }
        
        if error_message:
            custom_attrs["ErrorMessage"] = error_message
        
        # Start and log the step
...
def render_dynamic_update_form(
    ...
    with st.form(f"update_{table_name.lower()}_form"):
        ...
        for i, col_info in enumerate(editable_columns):
            ...
            current_value = record_data.get(normalized_col_name, "")
            if not current_value:
                # Fallback: try original column name
                current_value = record_data.get(col_name, "")
            if not current_value:
                # Fallback: try uppercase version
                current_value = record_data.get(col_name.upper(), "")
            if not current_value:
                # Fallback: try lowercase version
                current_value = record_data.get(col_name.lower(), "")
            ...
            with col1 if i % 2 == 0 else col2:
                if col_name.upper().endswith("CODE") and col_name.upper() in [
                    ...
                    if col_name.upper() == "PUBLISHERCODE":
                        ...
                        status_options = ["ACTIVE", "INACTIVE", "PENDING", "RESOLVED"]
                        current_index = (
