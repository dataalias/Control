CREATE procedure [ctl].[usp_InsertNewSubscription] (    
	...
	,@pIsActive					int					= 0
	,@pSubscriptionFilePath     varchar(255)
	,@pSubscriptionArchivePath     varchar(255)
	,@pSrcFilePath				varchar(255)
	,@pDestTableName			varchar(255)
	,@pFeedFormat				varchar(255)
...
select   @rows					= @@rowcount
        ...
        ,@ParametersPassedChar	= 
			'***** Parameters Passed to exec <schema>.usp_InsertNewSubscription' + @CRLF +
			...
			'    ,@pFeedFormat = ' + isnull(cast(@pFeedFormat as varchar(255)),'NULL') + @CRLF + 
